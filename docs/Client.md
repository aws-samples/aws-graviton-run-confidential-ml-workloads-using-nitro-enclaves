...
* **-s** or **--svc** : provide which AWS service to allow the enclave access to,
 currently only supports connections to KMS in region eu-west-1 (Ireland) over a
 fixed port: 8001

The proxy that will be created is a detached background process:
 *vsock-proxy 8001 kms.eu-west-1.amazonaws.com 443 --config network/proxy-config-kms.yaml*.
